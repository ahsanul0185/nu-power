...
const Header = () => {
  ...
  return (
    <header
      className={`fixed left-0 top-0 w-full px-6 sm:px-8 lg:px-16 flex justify-between md:block  z-50 duration-100 ${isScrolled ? "bg-white py-4 border-b border-b-gray/30" :  location.pathname === "/" ? "bg-transparent py-6 border-b-gray/0" : "bg-white py-4 border-b border-b-gray/30"}`}
    >
      <Link to="/">
      <img className={`${isScrolled || location.pathname !== "/" ? "w-32" : "w-36 lg:w-40"}`} src={isScrolled || location.pathname !== "/" ? logo_dark : logo_white} alt="Nu Power Group" /></Link>
      ...
      <nav className="hidden md:block absolute top-1/2 -translate-y-1/2 left-auto right-8 lg:right-auto lg:left-1/2 lg:-translate-x-1/2">
        <ul className={`font-semibold flex items-center gap-6 xl:gap-8  ${isScrolled ? "text-black xl:text-base" : location.pathname === "/" ? "text-white xl:text-xl" : ""}`}>
          {navItems.map((item, idx) => (
            <NavLink
              key={idx}
              to={item.route}
